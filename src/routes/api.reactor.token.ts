...
// TanStack Start server route — the port of happy-oyster's
// app/api/reactor/token/route.ts (Next.js). Same URL, same semantics, so the
// client's fetchToken() is unchanged.
...
async function mintToken(): Promise<Response> {
  ...
  const baseUrl =
    process.env.REACTOR_COORDINATOR_URL ||
    process.env.VITE_COORDINATOR_URL ||
