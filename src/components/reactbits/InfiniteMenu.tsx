...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    onActiveItemChange: (i: number) => void,
    onMovementChange: (m: boolean) => void,
    onInit: ((s: InfiniteGridMenu) => void) | null = null,
    scale = 1.0
  ) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  private initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        try { ctx.drawImage(img, x, y, cellSize, cellSize); } catch {}
      });
...
interface InfiniteMenuProps {
  items?: InfiniteMenuItem[];
  scale?: number;
  onActiveItem?: (item: InfiniteMenuItem) => void;
}
...
export default function InfiniteMenu({ items = [], scale = 1.0, onActiveItem }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const sketch = new InfiniteGridMenu(
      canvas, list, handleActiveItem, setIsMoving,
      (sk) => sk.run(), scale
    );
    ...
    <div className="relative h-full w-full">
      ...
      {activeItem && (
        <>
          ...
          <p className={`im-face-description ${isMoving ? "inactive" : "active"}`}>{activeItem.description}</p>
          {activeItem.link && (
            <button
              type="button"
              onClick={handleButtonClick}
              className={`im-action-button ${isMoving ? "inactive" : "active"}`}
              aria-label="Open link"
            >
              <span className="im-action-button-icon">&#x2197;</span>
            </button>
          )}
        </>
      )}
